...
def bilingual_from_card(card):
    """한국어 페이지 카드 안의 '한국어 제목 + 일본어 원문'을 직접 뽑는다."""
    lines=text_lines(card)
    ko_candidates=[]
    ja_candidates=[]
    ignore_ko=("스테이지 보기","통솔력","난이도","드롭","성 체력","스테이지 길이","적 라인","배경")
    for s in lines:
        if KO_RE.search(s) and not any(x in s for x in ignore_ko):
            ko_candidates.append(s)
        if JP_RE.search(s) and not KO_RE.search(s):
            # 중국어 안내문은 stage 제목 후보에서 제외
            if not any(x in s for x in ("目前","這個","個 stage","條件","解鎖","過關","重置","星級")):
                ja_candidates.append(s)
    ko=ko_candidates[0] if ko_candidates else ""
    ja=ja_candidates[0] if ja_candidates else ""
    return ja,ko
...
def main():
    ...
    payload={
        ...
        "generated_by":"scripts/update_stage_ko.py v3",
        "sheet_id":SHEET_ID,
